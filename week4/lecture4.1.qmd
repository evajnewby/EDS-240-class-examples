...
jobs_clean |>
  ...
  mutate(occupation = fct_reorder(occupation, total_earnings)) |>  # reorder occupation column by total earnings
  ggplot(aes(x = occupation, y = total_earnings)) +
  geom_col() +
  geom_text(aes(label = dollar(total_earnings)), hjust = 1.2, color = "white") +
...
jobs_clean |> 
  ...
  geom_text(aes(label = dollar(total_earnings)), hjust = -0.2) +
  scale_y_continuous(labels = label_currency(scale = 0.001, suffix = "k"), limits = c(0, 250000)) +
  coord_flip() +
  theme_minimal()
```
...
test <- jobs_clean |> 
  ...
  mutate(sex = str_remove(string = group, 
                          pattern = "total_earnings_")) %>% 
...
#| eval: true
#| echo: true
# create heatmap ---- (this looks funky)
